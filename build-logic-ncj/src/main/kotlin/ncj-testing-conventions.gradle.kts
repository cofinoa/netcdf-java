...
val testLtsVersions = listOf(minimumTestVersion, "11", "17", "21", "25")
...
val contentRootKey = "tds.content.root.path"
val isContentRootAvailable = isSystemPropertyAValidDirectory(contentRootKey)

val runAllTests = isSystemPropertySet("runAllTestsExceptIgnored")
val runSlowTests = isSystemPropertySet("runSlowTests")
val isRdaDataAvailable = isSystemPropertySet("rdaDataAvailable")
val isUcarNetworkAvailable = isSystemPropertySet("ucarNetworkAvailable")
val skipDockerTests = isSystemPropertySet("skipDockerTests")
    ...
    useJUnitPlatform {
      ...
      if (runAllTests) {
        ...
        if (!isContentRootAvailable && !isJenkins) {
          excludeTags("ucar.unidata.util.test.category.NeedsContentRoot")
          doFirst { logger.warn("Skipping all NeedsContentRoot tests.") }
        }

        if (isJenkins) {
          excludeTags("ucar.unidata.util.test.category.NotJenkins")
          doFirst {
            logger.warn(
              "Skipping all NotJenkins tests: detected that we're running in the Jenkins environment."
            )
          }
        }

        if (!runSlowTests) {
          excludeTags("ucar.unidata.util.test.category.Slow")
          doFirst { logger.warn("Skipping all Slow tests.") }
        }
        ...
        if (!isUcarNetworkAvailable) {
          excludeTags("ucar.unidata.util.test.category.NeedsUcarNetwork")
          doFirst { logger.warn("Skipping all tests that require access to the UCAR Network.") }
        }
        ...
        if (skipDockerTests) {
          ...
          doFirst { logger.warn("Skipping all tests that require docker.") }
        }
      }
    }
  }
